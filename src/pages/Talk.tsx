  ...
  useEffect(() => {
    ...
    recognition.onerror = (event: any) => {
      ...
      if (event.error === 'no-speech') {
        setPrompt('No speech detected. Listening again...');
        // Don't stop listening, let onend handle restart
      } else if (event.error === 'network') {
        setPrompt('Network error. Please check your connection.');
        // Continue listening for network errors
      } else if (event.error === 'not-allowed') {
        setPrompt('Microphone access denied. Please allow microphone access.');
        stopRecognition();
      } else if (event.error === 'audio-capture') {
        setPrompt('No microphone found. Please check your microphone.');
        stopRecognition();
      } else {
        setPrompt(`Recognition error: ${event.error}. Retrying...`);
